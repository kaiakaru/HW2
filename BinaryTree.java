...
public class BinaryTree {
    ...
    private void replaceValueHelper(Node node, int oldVal, int newVal) {
        ...
        // If node equals oldVal, replace with newVal
        if (node.data == oldVal) {
            node.data = newVal;
        }
    ...
    private int findMinHelper(Node node) {
        ...
        // ADD YOUR CODE HERE -- USE DEPTH FIRST SEARCH OF
        // If node is empty, return MAX_VALUE
        if (node == null) {
            return Integer.MAX_VALUE;
        }

        // Find min value in left and right subtree
    ...
    private int nodesGTHelper(Node node, int val) {
        ...
        // counter to keep track of amount of nodes greater than val
        int count = 0;
        if (node.data > val) {
            count++;
        }

        // Use recursion to count nodes greater than the value in left and right subtrees
    ...
    private int[] averageHelper(Node n) {
        // ADD YOUR CODE HERE -- USE DEPTH FIRST SEARCH OF
        // If node is null or tree is empty return 0 for sum and count
        if (n == null) {
            return new int[]{0,0};
        }

        // store sum and count of left and right subtrees
        int[] leftResult = averageHelper(n.left);
        int[] rightResult = averageHelper(n.right);

        // Calculate the sum of current node and its subtrees
